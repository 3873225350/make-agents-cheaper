...
\cref{tab:results-template} shows the minimum reporting table. A positive result requires cost and latency improvements without a task-success regression.
...
Uncached input tokens & 1.00$\times$ & 0.43$\times$ \\
Estimated cost & 1.00$\times$ & 0.57$\times$ \\
Median time to first token & 1.00$\times$ & 0.69$\times$ \\
